...
-- Player machines are the same prototypes on every planet, so this half of the death-spawn table
-- is not part of the swappable roster. See enemy_roster.lua for the enemy half.
local machine_spawn_map = {
...
-- Outpost turrets are player-tech entities sitting on the enemy force until the outpost is
-- captured, so their evolution payout is planet independent.
...
--- Pick a tier from a unit list scaled to the current evolution factor.
-- Rosters may have fewer (or more) tiers than the four Nauvis biters, so the list length drives
-- the scaling rather than a hard-coded 4.
local function get_level(list)
    local count = #list
    if count == 0 then
        return nil
    end
    ...
    return math.clamp(ceil(ef * count), 1, count)
end
...
local function do_coin_drop(entity_name, entity, cause)
    local position = entity.position
    local bounds = Roster.entity_drop_amount[entity_name]
    if not bounds then
        return
    end
...
local function do_spawn_entity(entity_name, entity, event)
    local spawn = Roster.enemy_spawn_map[entity_name] or machine_spawn_map[entity_name]
    if not spawn then
        return
    end
    ...
    if name == nil then
        ...
        if type == 'cause' then
            ...
            name = cause.name
            if not Roster.allowed_cause_source[cause.name] then
                return
            end
        elseif type == 'compound' then
            ...
            local list = Roster.unit_levels[type]
            local level = list and get_level(list)
            if not level then
                return
            end
            name = list[level]
